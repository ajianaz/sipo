...
class SatuanDao extends DatabaseAccessor<AppDatabase>
    with _$SatuanDaoMixin {
  SatuanDao(super.db);
  ...
  Future<bool> updateEntry(Satuan entry) =>
      update(satuans).replace(entry);

  Future<int> softDeleteEntry(int id) =>
      (update(satuans)..where((t) => t.id.equals(id)))
          .write(SatuansCompanion(isActive: const Value(0)));

  Future<int> deleteById(int id) =>
      (delete(satuans)..where((t) => t.id.equals(id))).go();

  Future<List<Satuan>> search(String query) => (select(satuans)
        ..where((t) => t.nama.contains(query) & t.isActive.equals(1)))
      .get();

  Future<int> toggleActive(int id) async {
    final satuan = await getById(id);
    return (update(satuans)..where((t) => t.id.equals(id)))
